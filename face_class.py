...
from flux_class_vecs import flux_cone,supp,get_efvs,get_gens,zero
import numpy as np
...
class Face:
    def __init__(self, model, rep_efv):
        ...
        self.S = np.r_[self.stoich,np.eye(len(rep_efv))[np.setdiff1d(supp(self.irr),model.irr_supp(rep_efv))]]
...
if __name__ == "__main__":
    ...
    counter2 = 0
    probs = []
    for candidate_ind,candidate in enumerate(face3_efvs):
        counter2 +=1
        face = Face(model,candidate)
        efvs = face.get_efvs()
        face_dims = [face.check_dim(efv) for efv in efvs]
        if  face_dims.count(3) == 2:
            more_than_two.append(face_dims.count(3))
            counter+=1
            in_a_3face = efvs[np.where(np.array(face_dims) == 3)[0]]
            if model.irr_supp(in_a_3face[0]) != model.irr_supp(in_a_3face[1]):
                
                print("here")
                print(candidate_ind)
                probs.append(candidate_ind)
        if face_dims.count(3) == 3:
            more_than_two.append(face_dims.count(3))
            counter+=1
            in_a_3face = efvs[np.where(np.array(face_dims) == 3)[0]]
