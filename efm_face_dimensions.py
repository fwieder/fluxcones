...
S = np.r_[model.stoich,nonegs]


def check(vector):
    return len(vector) - np.linalg.matrix_rank(S[zero(np.dot(S,vector))])
def check_rref(vector):    
    return len(vector) - len(Matrix(S[zero(np.dot(S,vector))]).rref()[1])
...
if __name__ == "__main__":
    ...
    dim_gens = list(tqdm.tqdm(map(check,model.generators), total = len(model.generators)))
    
    print(sorted(Counter(dim_gens).items()))
    ...
    print("Dimensions of faces all efvs are in with rref:")
    with Pool(12) as p:
        dims_efvs = list(tqdm.tqdm(p.imap(check_rref,model.generators),total= len(model.generators)))
    print(sorted(Counter(dims_efvs).items()))
    """    